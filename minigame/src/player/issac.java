...
//	private EnemyIssac enemyIssac;
	...
	public issac(miniApp app, Vector<Monster> monsters, Vector<structure> structures,
		...
		init(monsters, structures, startButtonControl, connectControl, timerControl);
		setting();
		batch();
		moveMotion();
	}

	public void init(Vector<Monster> monsters, Vector<structure> structures,
			StartButtonControl startButtonControl, ConnectControl connectControl, TimerControl timerControl) {
				...
				if (isLeft() == false) {
					...
					while (isLeft()) {
						...
						if (structures!=null) {
							for (int i = 0; i < structures.size(); i++) {
								if (!structures.get(i).isBroken() && structures.get(i).getSsStructure().getGubun() == "rock") {
									if (issac.getSsBody().getBounds().intersects(structures.get(i).getSsStructure().getBounds())) {
										setXPlayer(getXPlayer() + 2);
										issacInfoRefresh();
										isrock = true;
									}
